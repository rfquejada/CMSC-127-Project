...
CREATE VIEW estabsummary AS SELECT estabname, branch_address, rating FROM food_estab;


--View all food reviews for an establishment or a food item // yung review ba dito yung review body??
CREATE VIEW reviewssummary(userid, estabid, date_of_review, body, productid, rating) AS SELECT r.userid, r.estabid, r.date_of_review, r.body, r.productid, r.rating from user_reviews_foodestab_item r left join user_reviews_foodestab e on r.userid=e.userid and r.estabid=e.estabid UNION SELECT e.userid, e.estabid, e.date_of_review, e.body, r.productid, e.rating from user_reviews_foodestab_item r right join user_reviews_foodestab e on r.userid=e.userid and r.estabid=e.estabid and r.productid IS NULL;
